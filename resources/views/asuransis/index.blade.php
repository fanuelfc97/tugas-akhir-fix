...
@section('title', 'Data Polis Asuransi')
...
<div class="app-content-header"> <!--begin::Container-->
    <div class="container-fluid"> <!--begin::Row-->
        <div class="row">
            <div class="col-sm-6">
                <h3 class="mb-0">Data Polis Asuransi</h3>
            </div>
            <div class="col-sm-6">
        ...
        <div class="row">
            <div class="col-12">
                ...
                @if (in_array(Auth::user()->jabatan, ['Manager Administrasi', 'Staff Administrasi']))
                    <a href="{{ route('asuransis.create') }}" class="btn btn-primary mb-3">Tambah Asuransi</a>
                @endif
                    ...
                    <div class="modal fade" id="filterModal" tabindex="-1" aria-labelledby="filterModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('asuransis.generateReport') }}" method="GET">
                                ...
                                <tbody>
                                    @forelse($asuransis as $index => $asuransi)
                                        <tr>
                                            ...
                                            <!-- Periode Awal -->
                                            <td>{{ \Carbon\Carbon::parse($asuransi->periode_awalas)->format('d-m-Y') }}</td>

                                            <!-- Periode Akhir with color coding -->
                                            @php
                                                $periodeAkhir = \Carbon\Carbon::parse($asuransi->periode_akhiras);
                                            ...
                                            <td>
                                                @if ($asuransi->dokumen_polis)
                                                    <a href="{{ Storage::url($asuransi->dokumen_polis) }}" target="_blank">Lihat Dokumen</a>
                                                @else
                                                    Tidak ada dokumen
                                                @endif
                                            </td>
                                            <td>
                                                <!-- Detail button -->
                                                <a href="{{ route('asuransis.show', $asuransi) }}" class="btn btn-info btn-sm">Detail</a>
                                                @if (in_array(Auth::user()->jabatan, ['Manager Administrasi', 'Staff Administrasi']))
                                                <!-- Edit button -->
                                                <a href="{{ route('asuransis.edit', $asuransi) }}" class="btn btn-warning btn-sm">Edit</a>
                                                <!-- Delete button with form -->
                                                <form action="{{ route('asuransis.destroy', $asuransi) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
