...
@section('title', 'Data Bank Garansi')
...
<div class="app-content-header"> <!--begin::Container-->
    <div class="container-fluid"> <!--begin::Row-->
        <div class="row">
            <div class="col-sm-6">
                <h3 class="mb-0">Data Bank Garansi</h3>
            </div>
            <div class="col-sm-6">
        ...
        <div class="row">
            <div class="col-12">
                ...
                @if (in_array(Auth::user()->jabatan, ['Manager Administrasi', 'Staff Administrasi']))
                    <a href="{{ route('bankgaransis.create') }}" class="btn btn-primary mb-2">Tambah Bank Garansi</a>
                @endif
                    ...
                    <div class="modal fade" id="filterModal" tabindex="-1" aria-labelledby="filterModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('bankgaransis.generateReport') }}" method="GET">
                            ...
                            <table class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Dokumen Bank Garansi</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($bankgaransis as $index => $bankgaransi)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            ...
                                            <td>
                                                @php
                                                    $titikReklame = $titikReklames->firstWhere('koordinat_titik', $bankgaransi->koordinat_titik);
                                                @endphp
                                            ...
                                            <!-- Periode Awal -->
                                            <td>{{ \Carbon\Carbon::parse($bankgaransi->periode_awalbg)->format('d-m-Y') }}</td>

                                            <!-- Periode Akhir with color coding -->
                                            @php
                                                $periodeAkhir = \Carbon\Carbon::parse($bankgaransi->periode_akhirbg);
                                            ...
                                            <td>
                                                @if ($bankgaransi->dokumen_bg)
                                                    <a href="{{ Storage::url($bankgaransi->dokumen_bg) }}" target="_blank">Lihat Dokumen</a>
                                                @else
                                                    Tidak ada dokumen
                                                @endif
                                            </td>
                                            <td>
                                                <!-- Detail button -->
                                                <a href="{{ route('bankgaransis.show', $bankgaransi) }}" class="btn btn-info btn-sm">Detail</a>
                                                @if (in_array(Auth::user()->jabatan, ['Manager Administrasi', 'Staff Administrasi']))
                                                <!-- Edit button -->
                                                <a href="{{ route('bankgaransis.edit', $bankgaransi) }}" class="btn btn-warning btn-sm">Edit</a>

                                                <!-- Delete button with form -->
                                                <form action="{{ route('bankgaransis.destroy', $bankgaransi) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
